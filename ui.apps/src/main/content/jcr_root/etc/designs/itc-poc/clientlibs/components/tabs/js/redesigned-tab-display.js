...
(function () {
  ...
  Tabs = (function () {
    ...
    var templates = {
      tplTabNav: Handlebars.compile("<ul class='inline-list tabs-navigation show-desktop col-lg-12 col-md-12 col-sm-12 hidden-xs' role='tablist'>{{#each tab}}<li role='presentation' class='tab-menu-item'><button data-id='{{tabId}}' id='TabController-{{tabId}}' class='txt-btn' role='tab' aria-selected='false' aria-controls='TabController-{{tabId}}' tabindex><span class='titleSpan' >{{tabTitle}}</span></button></li>{{/each}}</ul>")
    };
    ...
    Tabs.prototype.createTabNav = function () {
      ...
      // add class to indicate that there's a navigation, as well as the length of the nav
      // length is needed so that via CSS, the font size can change if there are a ton of tab items
      // note this was a last-minute fix due to extra content added when Enterprise was released
      this.$container.addClass("tabs-nav-init").addClass("tabs-nav-init-len-" + this.$tabNavItems.length);
    };
      ...
      this.$tabNav.on("keydown", "button", function (e) {
        ...
        if (currentIndex !== null) {
          ...
          app.openTab($(document.getElementById(panelId)));
            //To focus on panel whose tab is active
            //check active element with active tab
            //focus on active tab panel
			//if(app.currentTab.$navItem.get(0)===document.activeElement.parentElement)
         //  {
		//		app.currentTab.$tab_panel.get(0).focus();
            //}else {
          		app.currentTab.$navItem.focus();// focus here so doesn't steal focus on page load
    ...
    Tabs.prototype.keyHandler = function (e) {
      ...
      switch (e.keyCode) {
        ...
        case keyCodes.UP:
          ...
          break;
		  /*when active tab is tabbed, it should select the tab panel and then move to the next inactive tab element*/
       // case keyCodes.TAB:
	//	   currentIndex = this.handleTab(currentIndex);
      //     break;
        case keyCodes.END:
          currentIndex = this.tabData.length - 1;
          break;
      ...
      return currentIndex;
      // pressing enter from screenreader doesn't work
      // space does
      // cant navigate via arrows alone in screenreader mode

    };
      Tabs.prototype.handleTab = function(currentIndex) {
          if(this.currentTab.$tab_panel.get(0)=== document.activeElement){
                currentIndex=this.currentTab.position + 1;
              }else{
              return currentIndex;
              }
      };
    ...
    Tabs.prototype.handleEnter = function(currentIndex) {
      ...
      var currentTabByFocusIndex = document.getElementById(document.activeElement.parentElement.getAttribute("aria-controls"));
        }else{
            var currentTabByFocusIndex = document.getElementById(document.activeElement.getAttribute("aria-controls"));
        }

       for( currentIndex =0; currentIndex < this.$tab_panels.length; currentIndex ++)
          {
              if(this.$tab_panels.eq(currentIndex).get(0).id===currentTabByFocusIndex.id.split('r-')[1])
			  {
                  return currentIndex;
              }
          }
     ...
     */
    Tabs.prototype.closeTab = function ($tabPanel) {
      var currentTab = this.currentTab;
    ...
    Tabs.prototype.openTab = function ($tab_panel) {
      ...
      this.currentTab = {
        ...
        $tab_panel.parent().show();
        $tab_panel.parent().siblings().find('.tabs-container__title').children('i').removeClass('fa-minus').addClass('fa-plus');
        $tab_panel.siblings().children('i').toggleClass('fa-plus fa-minus');
    ...
    Tabs.prototype.bindAccordionEvents = function () {
      ...
      this.$container.on("keydown", this.options.tab_class_title, function (e) {
        var currentIndex = app.keyHandler(e),
          focusIndex = document.activeElement;

        // compare current tab with the actually focused element
        if (app.currentTab.$title.get(0) !== focusIndex) {
    ...
    Tabs.prototype.toggleAccordion = function ($tab_panel) {
      ...
      // we never actually disable the current tab without opening a new one, otherwise on desktop you can have the scenario where there's no open tab
        ...
        this.updateAccordionAria($tabPanel);
        return false;
      }
     ...
     */
    Tabs.prototype.updateAccordionAria = function ($tabPanel) {
      if ($tabPanel.hasClass(this.options.accordion.closed_class)) {
        $tabPanel.attr({
          "aria-hidden": "true",
          "aria-expanded" : "false"
        });
        ...
        $tabPanel.parent().siblings().find('.tabs-container__title').children('i').removeClass('fa-minus').addClass('fa-plus');
        $tabPanel.siblings().children('i').toggleClass('fa-plus fa-minus');
    };
   ...
   */
    Tabs.prototype.openAccordion = function ($tabPanel) {
      ...
      $("html, body").animate({
        scrollTop: $tabPanel.offset().top - 120  
      }, 500);
      this.currentTab.$title.focus();
    };

      //This function close the current active tab in smalled device

     Tabs.prototype.closeCurrentAccordionTab = function(){

         if(this.currentTab.$tab_panel)
         {
             this.currentTab.$tab_panel.addClass(this.options.accordion.closed_class);
              this.currentTab.$tab_panel.attr({
              "aria-hidden": "true",
              "aria-expanded" : "false"
            });

             $('.tabs-container__panel--current').siblings().children('i').removeClass('fa-minus').addClass('fa-plus'); 
      ...
      Tabs.prototype.closeCurrentAccTab = function(){
          if(currentScope)
          {
			currentScope.closeCurrentAccordionTab();
          }
      }
      //end here
    ...
    Tabs.prototype.init = function () {
      ...
      //to close all the accordion tab at beginning 
        currentScope = this;
        if (! window.matchMedia("(min-width: 992px)").matches) {

			this.toggleAccordion($startingTab);  
            //this.closeCurrentAccTab();
        }
...
/*  $(function () {
    ...
    new Window.Tabs($($(".TabContainer").attr("role","tablist")[0]));
  });*/
	  $(function () {
		  Window.Tabs = Tabs;
		  $.each(  $(".TabContainer").attr("role","tablist") , function( index, value ) {
		    new Window.Tabs($(value));
		  });
	  });
...
$(document).ready(function(){
    ...
    var headingStyle = tileDialogConfig.isTarget;    
    $( ".tab-menu-item .titleSpan" ).wrap( "<"+headingStyle+" tabindex='0' role='tab'></"+headingStyle+">" );

    $('.tabs-init .accordion-wrapper').show();
        $('.tabs-navigation ').each(function() {
            var $AllLi  = $(this).children('li');
            var noLi    = $AllLi.length;
    		var itemSize    = Math.min(100, (10/noLi)*10);
            $AllLi.css('width', itemSize +'%');
           var elementHeights = $AllLi.find('.txt-btn').map(function() {
                return $(this).height();
              }).get();
              var maxHeight = Math.max.apply(null, elementHeights);
              $AllLi.find('.txt-btn').height(maxHeight);
	});
//after focus on active tab panel, it should focus the next tab (if any)
  /*$('.comp-tabdiv').on( 'keyup', function( e ) {
       if( e.which === 9 ) {
	 var active_Tab= document.querySelectorAll('[data-id='+document.activeElement.parentElement.id+']');
        if(active_Tab[0].parentElement.nextSibling !== null)
      {
       active_Tab[0].parentElement.nextSibling.firstElementChild.focus();
       }
    }
  });*/
    $(document).on('keydown','div.tabs-container__panel--current',function(e){
       if( e.target===this){
           if( e.shiftKey){
             //for shift+tab from panel, focus on tabName
		$('[data-id='+ this.id +']')[0].children[0].focus();
          }
          }
     });
   $('li.tab-menu-item h3').on('keyup',function(e){
       if(e.keyCode===16){
           if($(this).parent().hasClass('tabs-navigation__item--current') && $(this).parent()!==$('li.tab-menu-item').eq(0)){
               $(this).closest('.tab-menu-item').prev().find('h3').focus();
          }
        }
       if(e.keyCode===9){
           if($(this).parent().hasClass('tabs-navigation__item--current') && $(this).parent()!==$('li.tab-menu-item').eq(0)){
               $('#'+document.activeElement.parentElement.id.split('r-')[1])[0].focus();
           }
       }
    });
});
...
var lastWindowWidth;
    ...
    $( window ).resize(function(e) {
        if (window.matchMedia("(min-width: 992px)").matches) {
        	$('.tabs-container__panel--current').attr("aria-hidden","false");
        }
        if(window.matchMedia("(max-width: 991px)").matches && lastWindowWidth > 991){
            if(Window.Tabs !=undefined)
            {
                Window.Tabs.prototype.closeCurrentAccTab();
